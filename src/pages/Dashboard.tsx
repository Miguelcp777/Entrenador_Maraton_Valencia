...
import { getPhaseForDate, getDailyFocus, getMacrocycleProgress, getPhaseProgress, getWeeklyProgress, getWeekData } from '../utils/trainingLogic';
...
export default function Dashboard() {
    const { weight, setWeight, targetWeight, complianceScore, weeklyComplianceScore } = useAthlete();
    ...
    const focus = getDailyFocus(todayDate.getDay(), phase.name, todayDate);
    ...
    // Volumen semanal: km reales vs plan
    const currentWeek = getWeekData(todayDate);
    const planWeekKm = currentWeek?.volume ?? currentWeek?.longRun ?? 0;
    const [realWeekKm, setRealWeekKm] = useState(0);

    useEffect(() => {
        const fetchWeekKm = async () => {
            const monday = new Date();
            const cd = monday.getDay();
            monday.setDate(monday.getDate() - cd + (cd === 0 ? -6 : 1));
            monday.setHours(0, 0, 0, 0);
            const { data } = await supabase
                .from('logs_entrenamiento')
                .select('distancia_real_km')
                .gte('fecha_completada', monday.toISOString());
            if (data) setRealWeekKm(data.reduce((sum, l) => sum + (l.distancia_real_km || 0), 0));
        };
        fetchWeekKm();
    }, []);
    ...
    const handleSaveWeight = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (profile) {
                ...
                // Update profile current weight for global consistency
                await supabase.from('perfil_atleta').update({ peso_actual: kg }).eq('id', profile.id);
            }
            setWeight(kg); // refleja el peso actual sin recargar
            await fetchWeightLogs(); // actualiza la gráfica
        } catch (e) {
            console.error(e);
        }
        ...
        <div className="px-6 mt-8 max-w-4xl mx-auto space-y-10">
            ...
            <section className="flex flex-col items-center justify-center py-6">
                <div className="relative w-64 h-64 flex items-center justify-center">
                    ...
                    <svg className="absolute inset-0 w-full h-full transform -rotate-90">
                        ...
                        <circle className="text-primary transition-[stroke-dashoffset] duration-1000 ease-out" cx="128" cy="128" fill="transparent" r="120" stroke="currentColor" strokeDasharray="754" strokeDashoffset={754 * (1 - totalProg / 100)} strokeLinecap="round" strokeWidth="8"></circle>
                    </svg>
                    <div className="text-center space-y-1">
                        ...
                        <p className="font-['Space_Grotesk'] text-[10px] uppercase font-bold tracking-widest text-primary">DÍAS PARA LA META</p>
                    </div>
                </div>
                <div className="mt-4 flex gap-4">
                    <div className="text-center">
                        <span className="block font-['Space_Grotesk'] text-xl font-bold">{monthStr}</span>
                        <span className="block font-['Space_Grotesk'] text-[10px] text-zinc-500">MES</span>
                    </div>
                    <div className="text-center">
                        <span className="block font-['Space_Grotesk'] text-xl font-bold">{dayStr}</span>
                        <span className="block font-['Space_Grotesk'] text-[10px] text-zinc-500">DÍA</span>
                    </div>
                    <div className="text-center">
                        <span className="block font-['Space_Grotesk'] text-xl font-bold">{yearStr}</span>
                        <span className="block font-['Space_Grotesk'] text-[10px] text-zinc-500">AÑO</span>
                    </div>
                </div>
            </section>
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div className="bg-surface-container-low rounded-xl p-6 ambient-glow border border-primary/5">
                    <div className="flex justify-between items-start mb-6">
                        <span className="font-['Space_Grotesk'] text-[10px] uppercase font-bold tracking-widest text-zinc-500">Estado del Sistema</span>
                        <span className="material-symbols-outlined text-primary" style={{ fontVariationSettings: "'FILL' 1" }}>bolt</span>
                    </div>
                    <div className="flex items-end gap-3 mb-2">
                        <h2 className="font-['Inter'] font-black text-4xl uppercase italic">{isInactive ? 'INACTIVO' : 'ACTIVO'}</h2>
                    ...
                    <p className="text-zinc-400 text-sm font-['Inter'] leading-relaxed">
                        {isInactive
                            ? "Sistema en reposo. El protocolo se activará cuando empiece el plan (22 jun 2026)."
                            : `Hoy toca: ${focus.label}. Sigue el plan y fía la frecuencia cardíaca, no el ritmo.`}
                    </p>
                </div>
            ...
            <section className="bg-surface-container-low rounded-xl p-6 relative overflow-hidden">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <span className="font-['Space_Grotesk'] text-[10px] uppercase font-bold tracking-widest text-zinc-500">Composición Corporal</span>
                    ...
                    <div className="text-right">
                        <span className="font-['Space_Grotesk'] text-[10px] uppercase font-bold tracking-widest text-primary">Objetivo</span>
                        <div className="flex flex-col items-end gap-1">
                            ...
                            <span className="text-[10px] font-['Space_Grotesk'] font-bold uppercase tracking-widest text-zinc-500 bg-surface-container-high px-1.5 py-0.5 rounded">
                                {parseFloat(diff) > 0 ? `${diff} KG POR BAJAR` : 'OBJETIVO LOGRADO'}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <section className="bg-surface-container-low rounded-xl p-6 opacity-90 border border-white/5">
                <div className="flex items-center gap-2 mb-8">
                    ...
                    <h2 className="font-['Inter'] font-bold text-lg uppercase tracking-tighter">Progresión del Plan</h2>
                </div>
                    ...
                    {currentWeek && (
                        <div>
                            <div className="flex justify-between items-end mb-2">
                                <div>
                                    <span className="block font-['Space_Grotesk'] text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Volumen Semanal</span>
                                    <span className="block text-sm font-['Inter'] text-zinc-300">Km reales vs plan · Semana {currentWeek.n}/24</span>
                                </div>
                                <span className="font-['Inter'] font-black text-xl text-teal-400">{realWeekKm.toFixed(0)}<small className="text-zinc-500 font-['Space_Grotesk'] text-sm">/{planWeekKm} km</small></span>
                            </div>
                            <div className="h-2 w-full bg-surface-container-highest rounded-full overflow-hidden">
                                <div className="h-full bg-teal-400 rounded-full transition-all duration-1000 ease-out" style={{ width: `${planWeekKm ? Math.min(100, (realWeekKm / planWeekKm) * 100) : 0}%` }}></div>
                            </div>
                        </div>
                    )}
